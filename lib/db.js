...
export async function uploadOrderReceipt(orderId, imageFiles) {
  ...
  //   Object.keys(imageFiles).map((key) => console.log(imageFiles[key]));
  ...
  //     const fileRef = storage.ref().child(`${orderId}-${file.name}`);
  //   await fileRef.put(file);
  //   const fileUrl = await fileRef.getDownloadURL();
  //   imageURLs.push(fileUrl);
  //   });
...
export async function updateOrderReceipt(orderId, files) {
  const imageUrls = await uploadOrderReceipt(orderId, files);
  console.log(imageUrls);
}
